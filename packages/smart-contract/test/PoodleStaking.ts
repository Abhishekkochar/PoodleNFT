...
describe('Staking', () => {
	describe('Deployment', () => {
		...
		it('Should have the right WITHDRAW_ROLE', async () => {
			...
			expect(
				await stake.hasRole(role, poodleDeployment.emergencyWithdraw.address)
			).to.be.equal(true);
		});
	});
	describe('Staking', () => {
		it('Should successfully stake Poodle nft', async () => {
			...
			await expect(stake.connect(userOne).stakeNft(0))
				.to.emit(stake, 'STAKE')
		...
		it('Should not able to stake if not right owner', async () => {
			const { poodleDeployment, stake } = await loadFixture(deployStaking);
			...
			await poodle.connect(userOne).approve(await stake.getAddress(), 0);
			// Staking, Checking for staking events
			await expect(stake.connect(userTwo).stakeNft(0)).to.be.rejectedWith(
		...
		it('Should successfully revert if the Poodle nft is not valid', async () => {
			const { stake } = await loadFixture(deployStaking);
			// Staking, Checking for staking events
			await expect(stake.stakeNft(0)).to.rejectedWith(
	...
	describe('UnStaking', () => {
		it('Should successfully unStake Poodle nft', async () => {
			...
			await expect(
				stake.connect(userTwo).unStakeNft(0)
			).to.be.revertedWithCustomError(stake, 'NOT_AUTHORIZED');
			// UnStaking from userOne
			await expect(stake.connect(userOne).unStakeNft(0))
				.to.be.emit(stake, 'UNSTAKE')
				.withArgs(userOne.address, 0);
		...
		it('Should successfully unStake multiple nfts only by address with the role `WITHDRAW_ROLE`', async () => {
			...
			await expect(stake.connect(emergencyWithdraw).forcedUnStake([0, 1]))
				.to.be.emit(stake, 'EMERGENCYUNSTAKE')
				.withArgs(emergencyWithdraw.address, [0, 1]);
		});
	});
});
